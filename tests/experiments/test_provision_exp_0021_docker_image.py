...
class Tests(unittest.TestCase):
 ...
 def test_transfer_tags_loaded_image_for_expected_docker_tag(self):
  p=m.load(); d=p["docker_image"]
  source={"Id":p["source_podman_image"]["id"].removeprefix("sha256:"),"Os":"linux","Architecture":"amd64","Config":{"Entrypoint":d["entrypoint"]}}
  loaded={"Id":d["id"],"Os":"linux","Architecture":"amd64","Config":{"Entrypoint":d["entrypoint"]}}
  run=MagicMock(side_effect=[MagicMock(returncode=0,stdout=__import__('json').dumps([source])),MagicMock(returncode=0),MagicMock(returncode=0,stdout=__import__('json').dumps([loaded])),MagicMock(returncode=0)])
  ...
  self.assertIn((['docker','tag',d['id'],d['tag']],),[call.args for call in run.call_args_list])
