#!/usr/bin/env python3
"""Validate frozen EXP-0010 evidence against its historical Git preimage."""
...
RESULT_PATH = ROOT / "experiments" / "ebook" / "exp-0010" / "result.json"
EXPECTED_PREIMAGE_COMMIT = "fbb481b4e5f869943c0a668502d37e867b2db5ce"
...
def load_runner() -> ModuleType:
    spec = importlib.util.spec_from_file_location(
        "sammlungslotse_exp_0010_historical",
        RUNNER_PATH,
    )
    if spec is None or spec.loader is None:
        raise HistoricalValidationError("EXP-0010 runner cannot be loaded")
...
def git_bytes(*arguments: str) -> bytes:
    ...
    if completed.returncode != 0:
        ...
        raise HistoricalValidationError(
            f"Git history cannot provide EXP-0010 evidence: {detail or arguments[0]}"
        )
...
def ensure_ancestor(commit: str) -> None:
    ...
    if completed.returncode != 0:
        raise HistoricalValidationError(
            "EXP-0010 preimage commit is unavailable or not an ancestor; full Git history is required"
        )
...
def historical_preimage(result: dict[str, Any]) -> dict[str, str]:
    ...
    if not isinstance(preimage, dict):
        raise HistoricalValidationError("EXP-0010 preimage record is missing")
    ...
    if not isinstance(authority, dict) or not isinstance(expected, dict):
        raise HistoricalValidationError("EXP-0010 preimage binding is incomplete")
    commit = authority.get("preimage_commit")
    if commit != EXPECTED_PREIMAGE_COMMIT or not re.fullmatch(r"[0-9a-f]{40}", str(commit)):
        raise HistoricalValidationError("EXP-0010 preimage commit differs")
    if set(expected) != set(RUNNER.PREIMAGE_FILES):
        raise HistoricalValidationError("EXP-0010 preimage locator set differs")
    ...
    if actual != expected:
        raise HistoricalValidationError("EXP-0010 historical preimage hashes differ")

    for locator in FROZEN_EXPERIMENT_LOCATORS:
        path = ROOT / locator
        if not path.is_file() or RUNNER.sha256_file(path) != expected[locator]:
            raise HistoricalValidationError(
                f"EXP-0010 frozen experiment file differs: {locator}"
            )
    return actual
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 1
    print(
        f"EXP-0010 historical result valid: {sum(result['acceptance'].values())}/"
