#!/usr/bin/env python3
"""Validate frozen EXP-0011 evidence against its historical Git preimage."""
...
EXPECTED_PREIMAGE_COMMIT = "a5aeb0196d8d6a32fc90da46ca158ba693c6a0db"
...
def load_runner() -> ModuleType:
    spec = importlib.util.spec_from_file_location(
        "sammlungslotse_exp_0011_historical",
        RUNNER_PATH,
    )
    if spec is None or spec.loader is None:
        raise HistoricalValidationError("EXP-0011 runner cannot be loaded")
...
def git_bytes(*arguments: str) -> bytes:
    ...
    if completed.returncode != 0:
        ...
        raise HistoricalValidationError(
            f"Git history cannot provide EXP-0011 evidence: {detail or arguments[0]}"
        )
...
def ensure_ancestor(commit: str) -> None:
    ...
    if completed.returncode != 0:
        raise HistoricalValidationError(
            "EXP-0011 preimage commit is unavailable or not an ancestor; full Git history is required"
        )
...
def historical_preimage(result: dict[str, Any]) -> dict[str, str]:
    ...
    if not isinstance(expected, dict) or set(expected) != set(RUNNER.PREIMAGE_FILES):
        raise HistoricalValidationError("EXP-0011 preimage locator set differs")
    if commit != EXPECTED_PREIMAGE_COMMIT or not re.fullmatch(r"[0-9a-f]{40}", str(commit)):
        raise HistoricalValidationError("EXP-0011 preimage commit differs")
    if result.get("authority", {}).get("preimage_commit") != commit:
        raise HistoricalValidationError("EXP-0011 authority commit differs")
    ...
    if actual != expected:
        raise HistoricalValidationError("EXP-0011 historical preimage hashes differ")

    for locator in FROZEN_EXPERIMENT_LOCATORS:
        path = ROOT / locator
        if not path.is_file() or RUNNER.sha256_file(path) != expected[locator]:
            raise HistoricalValidationError(
                f"EXP-0011 frozen experiment file differs: {locator}"
            )
    return actual
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 1
    print(
        f"EXP-0011 historical result valid: {sum(result['acceptance'].values())}/"
        f"{len(result['acceptance'])} cases={len(result['cases'])}"
    )
    return 0
